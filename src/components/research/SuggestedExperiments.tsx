  ...
  if (experiments.length === 0) {
    return (
      <div className="rounded-xl border border-border/60 bg-card p-5">
        <h3 className="text-[13px] font-semibold text-foreground/80 tracking-wide mb-1">
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card p-5">
      ...
      <div className="space-y-3">
        {experiments.map((e, i) => (
          <div key={i} className="rounded-lg border border-border/40 bg-muted/5 p-3">
            ...
            <p className="text-[11px] text-muted-foreground/80 mt-1.5 leading-snug">{e.rationale}</p>
          </div>
        ))}
      </div>
    </div>
