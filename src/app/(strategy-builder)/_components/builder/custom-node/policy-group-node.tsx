...
import type { PolicyGroupNodeData, CompilerDiagnostic } from "@/types/strategy-builder";

interface PolicyGroupNodeProps {
  id: string;
  data: PolicyGroupNodeData;
  selected?: boolean;
}

export default React.memo(function PolicyGroupNode({ id, data, selected }: PolicyGroupNodeProps) {
  const { label } = data || {};
  ...
  const policiesCount = data.policies ? data.policies.length : 0;
  const nodeLabel = label || "Risk Policy Group";
    ...
    >
      ...
      <div
        className={`
          ...
          rounded-tl-xl rounded-bl-xl rounded-br-xl p-4 w-72 min-h-[6rem] h-auto shadow-md transition-all duration-300
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            <div className="size-8 bg-orange-600 text-white rounded-md flex items-center justify-center">
              <IconShield className="size-4" />
            </div>
            <div className="flex flex-col select-none">
              <h3 className="text-foreground font-semibold text-sm truncate max-w-[150px]">
                {policiesCount === 0 ? "Unconfigured Node" : nodeLabel}
              </h3>
              {policiesCount === 0 ? (
          ...
          <div className="flex items-center gap-1 select-none">
            <Badge variant="outline" className="text-[10px] border-orange-500/30 text-orange-600 dark:text-orange-400 bg-orange-500/10">
              Risk
            </Badge>
          </div>
        </div>
        {nodeDiagnostics.length > 0 && (
          <div className={`text-[10px] font-medium truncate max-w-[240px] mt-1 select-none ${
        ...
        {(isHovered || selected) && (
          <div 
            className={`absolute -right-px top-[-25px] h-[26px] bg-white dark:bg-[#1B1D21] border border-b-0 rounded-t-lg px-1.5 flex items-center gap-1 z-40 animate-in fade-in slide-in-from-bottom-1 duration-150 ${
              selected ? "border-orange-500 shadow-[0_-3px_8px_rgba(249,115,22,0.15)]" : "border-border shadow-xs"
            }`}
      ...
      <Handle
        ...
        className="w-2.5! h-2.5! bg-white! border-2! border-orange-500! rounded-full!"
        style={{ top: -5 }}
      />
      <Handle
        type="source"
